...
void GSM::InitSerLine(long baud_rate)
{
  ...
  for (int i=1;i<7;i++){
	  ...
	  Serial2.write("AT+IPR=");
	  Serial2.write(baud_rate);    
	  Serial2.write("\r"); // send <CR>
  }
...
void GSM::Call(int sim_position)
{
  ...
  SetCommLineStatus(CLS_ATCMD);
  // ATD>"SM" 1;<CR>
  Serial2.write("ATD>\"SM\" ");
  Serial2.write(sim_position);    
  Serial2.write(";\r");
...
char GSM::SetSpeakerVolume(byte speaker_volume)
{
  ...
  // select speaker volume (0 to 14)
  // AT+CLVL=X<CR>   X<0..14>
  Serial2.write("AT+CLVL=");
  Serial2.write((int)speaker_volume);    
  Serial2.write("\r"); // send <CR>
  // 10 sec. for initial comm tmout
  // 50 msec. for inter character timeout
...
char GSM::SendDTMFSignal(byte dtmf_tone)
{
  ...
  SetCommLineStatus(CLS_ATCMD);
  // e.g. AT+VTS=5<CR>
  Serial2.write("AT+VTS=");
  Serial2.write((int)dtmf_tone);    
  Serial2.write("\r");
...
char GSM::InitSMSMemory(void) 
{
  ...
  SendATCmdWaitResp("AT+CNMI=2,1,0,0,0", 1000, 50, "OK", 2);
...
char GSM::IsSMSPresent(byte required_status) 
{
  ...
  switch (required_status) {
    case SMS_UNREAD:
      Serial2.write("AT+CMGL=\"REC UNREAD\"\r");
      break;
    case SMS_READ:
      Serial2.write("AT+CMGL=\"REC READ\"\r");
      break;
    case SMS_ALL:
      Serial2.write("AT+CMGL=\"ALL\"\r");
      break;
  }
...
char GSM::GetSMS(byte position, char *phone_number, char *SMS_text, byte max_SMS_len) 
{
  ...
  //Serial2.write((int)position);  
  char posArray[4];
  itoa(position, posArray, 10);
  Serial2.write(posArray); // was : Serial2.write((int)position)
  Serial2.write("\r");
...
char GSM::WritePhoneNumber(byte position, char *phone_number)
{
  ...
  Serial2.write("AT+CPBW=");
  Serial2.write((int)position);  
  Serial2.write(",\"");
  Serial2.write(phone_number);
  Serial2.write("\"\r");
